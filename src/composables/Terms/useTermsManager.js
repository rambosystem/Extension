...
export function useTermsManager() {
  ...
  const refreshTerms = async (showSuccessMessage = true) => {
    ...
    try {
      ...
      if (hasSuccess && showSuccessMessage) {
        ElMessage.success(
          t("terms.refreshSuccess") || "Terms refreshed successfully"
        );
      }
  ...
  const rebuildEmbedding = async () => {
    try {
      ...
      // 显示成功提示
      ElMessage.success(
        t("terms.rebuildStarted") || "Embedding rebuild started successfully"
      );
      ...
      if (err.message.includes("Index Processing")) {
        ElMessage.warning(
          t("terms.rebuildEmbeddingProcessing") ||
            "Index Processing, please try again later"
        );
      } else {
  ...
  const addTerms = async (newTermsData) => {
    ...
    try {
      ...
      ElMessage.success(
        t("terms.addSuccess", { count: addedCount }) ||
          `成功提交${addedCount}个terms更新至数据库`
      );
  ...
  onMounted(() => {
    ...
    Promise.allSettled([fetchTermsStatus(), fetchEmbeddingStatus()]).then(
      (results) => {
        // 记录每个请求的结果
        results.forEach((result, index) => {
          const requestName = index === 0 ? "Terms Status" : "Embedding Status";
