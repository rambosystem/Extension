<template>
  ...
  >
    ...
    <el-form label-position="top">
      <el-form-item class="excel-wrapper-item">
 ...
 * 格式：小�?+ 下划线，例如 "Chinese" -> "chinese", "Spanish (Latam)" -> "spanish_(latam)"
...
/**
 * 格式化字段名为显示名�?
 * @param {string} fieldName - 字段名（�?"chinese", "japanese"�?
...
/**
 * 根据翻译结果中的 targetLanguages 确定列顺�?
 * 列顺序：key, en, 然后按照翻译时的 targetLanguages 的顺�?
 * @returns {string[]} 字段名数�?
 */
const extractFieldNames = () => {
  const fieldNames = ["key", "en"]; // 固定前两�?
...
const calculatedColumnWidth = computed(() => {
  ...
  // - 行号列宽�? 40px
  // - 垂直滚动条宽�? 17px（预留，避免最右边列被垂直滚动条遮挡）
  const containerPadding = 20; // excel-container 左右 padding
  const rowNumberWidth = 40; // 行号列宽�?
  ...
  // 注意：这里返回的是其他列的宽度，Key列宽度需要在Excel组件中特殊处�?
  return {
    key: keyColumnWidth,
...
const updateDialogWidth = () => {
  nextTick(() => {
    if (excelWrapperRef.value) {
      ...
      // 确保 dialogElement �?DOM 元素且有 querySelector 方法
      if (
        dialogElement &&
...
const convertFromExcelData = (excelDataArray) => {
  ...
  // 保留所有行（包括空行），因为用户可能正在编辑或刚插入新�?
  // 只在导出或保存时过滤空行，而不是在每次数据变化时过�?
...
const handleExcelDataChange = (data) => {
  ...
  if (isKeyColumn) {
    nextTick(() => {
      ...
      if (updated) {
        // 如果更新了key，setData 会自动触�?change 事件
        // 设置标志，避免循环更�?
        isUpdatingKeyOrder = true;
        nextTick(() => {
          isUpdatingKeyOrder = false;
        });
        // 不需要在这里再次处理数据，因�?setData 会触�?change 事件
        return;
      }
    });
  }
...
/**
 * 自定义菜单项配置
 * 只在列名�?"Key" �?Auto-Increment 开关开启时显示自增填充菜单�?
 */
const customMenuItems = computed(() => {
  // 检�?Auto-Increment 开关是否开�?
...
const generateNextAvailableKey = (baselineKey, usedKeys) => {
  ...
  // 如果没有已使用的数字，从 baseline number 开�?
  ...
  // 查找缺失的数�?
  // 搜索范围：从 baselineNumber 开始，�?maxUsedNumber
  // 最小值一定是 baselineNumber（baseline key 的数字部分）
  const missingNumbers = [];

  // 如果 maxUsedNumber < baselineNumber，说�?baselineNumber 本身就是一个缺失的数字
  ...
  // �?baselineNumber �?maxUsedNumber 之间查找缺失的数�?
  ...
  // 如果没有缺失的数字，使用下一个递增的数�?
  // �?maxUsedNumber + 1 �?baselineNumber 中的较大值（确保不小�?baselineNumber�?
...
const handleCustomAction = ({ id, context }) => {
  if (id === "auto-increment-key") {
    ...
    // 如果有符合条件的单元格，按顺序重新生成所�?key
    if (targetRows.length > 0) {
      // 先清除这些行的现�?key（如果符合格式），避免影响唯一性检�?
