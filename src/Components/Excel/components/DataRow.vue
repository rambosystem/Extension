  ...
  <div class="excel-row">
    ...
    >
      ...
      <RowResizer
        ...
        @mousedown.stop="$emit('row-resize-start', rowIndex, $event)"
    ...
    <ExcelCell
      ...
      :row-index="rowIndex"
      :col-index="colIndex"
      :cell-value="tableData[rowIndex]?.[colIndex]"
      :is-last-column="colIndex === columns.length - 1"
      ...
      :is-drag-target="isInDragArea(rowIndex, colIndex)"
      ...
      :multiple-drag-border-class="
        getMultipleDragBorderClass(rowIndex, colIndex)
      "
      :drag-target-border-class="getDragTargetBorderClass(rowIndex, colIndex)"
      :cell-display-style="getCellDisplayStyle(rowIndex, colIndex)"
      :column-width="getColumnWidth(colIndex)"
      ...
      @cell-menu-command="
        (cmd) => {
          const normalized =
            typeof cmd === 'string' ? { action: cmd, rowIndex } : cmd;
