<template>
  <el-dialog :modelValue="translationCoreStore.dialogVisible" @update:modelValue="translationCoreStore.setDialogVisible"
    :title="t('translation.translationResult')" width="70%">
    <el-form label-position="top">
      <el-form-item>
        <div v-loading="translationCoreStore.loadingStates.translation"
          :element-loading-text="translationCoreStore.getStatusText()" class="excel-wrapper">
          <Excel v-model="excelData" @change="handleExcelDataChange" :enableColumnResize="true" :enableRowResize="false"
            :enableFillHandle="true" :defaultColumnWidth="200" :columnNames="getColumnConfig.columnNames" />
        </div>
      </el-form-item>
...
watch(
  () => exportStore.targetLanguages,
  () => {
    excelData.value = convertToExcelData(translationCoreStore.translationResult);
  }
);
